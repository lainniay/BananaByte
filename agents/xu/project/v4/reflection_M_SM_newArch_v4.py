...
# 确保导入了核心组件
from core import GeminiLLM, ImageContent, Message, OpenAILLM, TextContent
...
def _get_expert_feedback(
    ...
    try:
        config = OpenAILLM.GenerateConfig(temperature=1.0)
...
def handle_imagine(ctx: AgentContext) -> State:
    """阶段 1: 反射想象补全."""
    print("\n--- 🟢 阶段 1: 🎃 [imagine_background] 正在生成纯反射底图... ---")
...
def handle_edit(ctx: AgentContext) -> State:
    ...
    refine_prompt = f"上一轮不理想。第一张图是原图，第二张图是去除反射后目前的效果。根据专家反馈修改第二张图。专家反馈：\n{ctx.latest_feedback}\n"
...
def handle_evaluate(ctx: AgentContext) -> State:
    ...
    seg_result: str = ctx.seg_result_txt

    # 分别获取背景和光学的反馈
    bg_critique = _get_expert_feedback(
        ctx.critic_client,
        ctx.current_image,
        "你是物体逻辑专家。检查物体遮挡是否自然？若完美请回‘无需改进’。",
    )
    light_critique = _get_expert_feedback(
        ctx.critic_client,
        ctx.current_image,
        f"你是图像融合专家。检查除语义分割: {seg_result} 的 [main object] 外有无不自然的衔接？若完美请回‘无需改进’。",
    )
...
def main() -> None:
    ...
    # 1. 初始化模型
    image_llm = GeminiLLM(model="gemini-2.5-flash-image")
    critic_llm = OpenAILLM(model="kimi-k2.5")

    # 2. 读取并处理图像
    input_path = Path(
        r"D:\Develop\BananaByte\workspace\Reflection_Removal\real20_420\blended\39.jpg"
    )
    ...
    seg_rs_txt: str = """
    [
        {"box_2d": [426, 178, 992, 1000], "label": "main object"},
        {"box_2d": [0, 0, 560, 1000], "label": "reflective layer"}
    ]
    """
